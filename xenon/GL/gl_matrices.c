...
xe_matrix_t projection_matrix = {1, 0, MATPROJECTION};
xe_matrix_t modelview_matrix = {1, 0, MATMODELVIEW};
...
void glMatrixMode (GLenum mode)
{
	...
	{
		case GL_MODELVIEW:
			current_matrix = &modelview_matrix;
			break;

		case GL_PROJECTION:
			current_matrix = &projection_matrix;
			break;
...
void XeGlCheckDirtyMatrix(xe_matrix_t *m){
	if (m->dirty)
	{
		Xe_SetVertexShaderConstantF(xe, m->usage, (float*)m->stack[m->stackdepth].f, 4);
		m->dirty = 0;
	}
}
